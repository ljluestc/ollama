...
- **Fix:** Resolved build issue with `int` vs `uint64` comparison in `llm/server.go`.
- **Fix:** Improved macOS `startApp` logic to gracefully handle local non-app-bundle binaries.
...
3.  **Run with `--fit-vram` (Auto-sizing):**
    Run a model and verify the logs show the context being adjusted.
    ```bash
    ./ollama run llama3 --fit-vram
    ```
    *Verify in logs:* Look for `auto-sized num_ctx` in the server logs.

4.  **Run with `--max-vram` (Strict Limit):**
    Set a low limit (e.g., 2GB or 4GB) to force a smaller context than the machine supports.
    ```bash
    ./ollama run llama3 --fit-vram --max-vram=4GB
    ```
    *Verify behavior:* The context size should be significantly smaller than default (8192) and fit within the 4GB limits.
